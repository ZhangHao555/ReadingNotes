...
你有一些领域数据置身于GUI控件中，而领域函数需要访问这些数据。将改数据赋值到一个领域对象中。建立一个Observer模式，用以同步领域对象和GUI对象内的重复数据。

#### 动机 
一个良好的分层系统 应该将处理用户界面和处理业务逻辑的代码分开。
...
你的类中存在一个public 字段，将它圣母为private，并提供相应的访问函数。
